...
def add(date: str, number: int):
    ...
    return f'Добавлено {number} рублей'
...
def calculate_year(year: int):
    ...
    if year in storage:
        ...
        return f'Потрачено за {year} год {expense} рублей'
    return f'{year}г. не найден'
...
def calculate_month(year: int, month: int):
    ...
    if year in storage and month in storage[year]:
        expense += storage[year][month]
        return f'Потрачено за {month} месяц {year} года: {expense} рублей'
    else:
        return f'не найдено'
