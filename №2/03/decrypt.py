...
def decrypt(encryption: str) -> str:
    ...
    while i < len(encryption):
        if encryption[i] == '.':
            # Если текущий символ - точка, проверяем следующий символ
            if i + 1 < len(encryption) and encryption[i + 1] == '.':
                # Если следующая точка тоже точка, удаляем последний символ из стека, если он есть
                if stack:
                    stack.pop()
                # Пропускаем две точки
                i += 1
            else:
                # Если только одна точка, просто пропускаем её
                pass
        else:
            # Если это не точка, добавляем символ в стек
            stack.append(encryption[i])
        i += 1

    # Объединяем стек в строку
    return ''.join(stack)
